...
pub fn parse_content(x: Node) -> Result<Option<ContentType>, Box<dyn std::error::Error>> {
    let class = x.attr("class").unwrap_or("");
    let _type = get_content_type(class);
    if _type.is_none() {
        Ok(None)
    } else {
        match _type.unwrap() {
            ...
            Type::Image => {
                let result = x.find(Name("img")).next().and_then(|n| n.attr("src")).map(|s| ContentType::Image { src: s.to_string() });
                Ok(result)
            },
            Type::Spoiler => {
